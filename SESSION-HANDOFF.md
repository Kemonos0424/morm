...
- ✅**[完了 2026-08-29・最後の1つ]** **新 wMORM/USDC の Uniswap v3 プール作成＋UI移行 完了**。プール=`0xcbC9fe1fA344036146949165e935cada066F9E45`(fee **1%**/tickSpacing200・full-range・**100,000 wMORM + 1,000 USDC @ $0.01**)。★**token順序反転の罠**: 新 WMORM `0x91FF…` < USDC `0xcC07…` で **wMORM=token0**(旧は USDC=token0)→ `SeedPool.s.sol` を順序対応(`sqrtP=7922816251426433759354`・fee10000・TL/TU=∓887200)、`index.html`/`app.html` の `priceFromSqrt` を `praw*1e12`(旧`1e18/praw/1e6`)へ・volume を amount1・POOL/POOL_BLOCK(46111394)/FEE(10000)/UI「手数料1%」更新→Mac Mini 再scp。**検証**: https://market.morm.one/ が **$0.010000・TVL$2,000・プール100,000wMORM/$1,000USDC** を実プール直読み表示、app.html はコンソールエラーなし・手数料1%・新wMORM/Bridge。※誤作成の `market.morm.one.ctai.online` は無害だが掃除推奨(ctai.online ゾーン)。
