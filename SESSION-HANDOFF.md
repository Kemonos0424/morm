...
- **contract MIN_EXIT ★再デプロイ要**: 稼働中 MORMExportBridge `0xf7a4c27a…db10a818`(Base Sepolia)の `minExit=0`(無効)を確認。しかし `minExit` は **immutable**(setter無し)＝**このコントラクトでは変更不可**。1e18適用には **_minExit=1e18 での再デプロイ＋移行**(BRIDGE_ADDR全面差替: relayer env/.testnet-deploy.env/morm-market app.html 等、WMORM mint権限の新bridgeへの移譲)が必要。testnet(実資金なし)・relayer側は dust ALERT で暫定緩和済。**要ユーザー判断(deployer鍵・移行)**。context: threshold=2・paused=false・maxMintPerWindow=1e24。
