#!/bin/bash
# Phase 2-② proportional-settlement verification. Ephemeral L1 (8904) + play_server
# settle_points() driven directly (no HTTP server). Own treasury, temp DB.
...
DATADIR=$(mktemp -d "$SCRATCH/morm-p2p-XXXXXX")
SEEDFILE=$(mktemp "$SCRATCH/p2p-seed-XXXXXX"); printf '%s' "$SEED" > "$SEEDFILE"
PLAYDB="$SCRATCH/p2p-play-$$.db"; rm -f "$PLAYDB"
...
PYTHONPATH="/Users/akihisayachida/Desktop/MORM/morm-play:$L1DIR" \
  python3 "$LANE/phase2_prop_client.py"
RC=$?
[ "$RC" != "0" ] && { echo "---- l1 log ----"; tail -15 "$SCRATCH/p2p-l1.log"; }
exit $RC
