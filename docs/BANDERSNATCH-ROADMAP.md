...
### D-2 ★適法性の建て付け — 【決定 2026-09-04：オフショア・ジオブロック型】
※以下は要件整理・設計であって法的助言ではない。**iGaming/暗号資産ゲーミング専門の弁護士＋ライセンス法人ストラクチャリングを着手前提**とする（本節はそのインプット）。

#### 法令マップ（実質判断の引き金）
| 法令 | 成立要件 | 引き金 |
|---|---|---|
| 賭博罪(刑185-186) | 偶然の勝敗で財産上の利益の得喪を争う／当事者が自分の財産を賭ける。運営が場を提供し利得＝賭博場開帳図利(重い) | **有料エントリー** |
| 富くじ(刑187) | 券を販売し抽選で購入者に配分・購入者が損失リスク | **チケットの有償販売＝Mad Hatter有償転売** |
| 景表法(懸賞) | 賞品が取引に付随で上限(一般懸賞10-20倍・上限10万・総額売上2%)。**取引非付随=オープン懸賞は上限なし** | NODE購入がエントリー対価化 |
| 資金決済法(暗号資産交換業) | MORMの売買/交換/管理(カストディ)を業として=登録制 | 運営がMORMをエスクロー保有し市場運営 |

**核心の分岐点**: 「参加者が自分の財産を賭けて(対価を払って)チャンスを得るか」。無料+運営拠出=懸賞(合法)、有料でチャンス購入=賭博/富くじ(刑事)。**主犯はMad Hatterの有償転売**。

#### 採用姿勢：オフショア・ジオブロック型
有償チケット設計を維持し、**ライセンス法域の法人＋KYC/AML＋禁止法域ジオブロック＋責任あるゲーミング**で運用。循環は3姿勢中で最強（下記）。

#### ★正直な限界（弁護士に必ず確認すべき芯）
1. **★JP在住の運営者/創業者は、オフショア法人を挟んでも日本刑法(賭博場開帳図利)が及びうる**。実質の運営支配が日本にあれば日本で処罰対象になり得る。事業体・運営支配の所在設計は専門弁護士＋法人ストラクチャリング領域（本ロードマップ範囲外）。
2. ジオブロックは**禁止法域ユーザーを入れないコンプラ制御**として設計（回避手段は設計しない）。ブロック不十分だと当該法域の執行が運営に及ぶ。
3. MORM自体の暗号資産該当(資金決済法)・オフランプ銀行アクセス・税務・制裁(OFAC等)が別途かかる。

#### 追加コンプラ層（要件）
- ライセンス法域での法人設立＋ゲーミング/くじライセンス取得。
- **KYC/AML**: 本人確認・制裁スクリーニング・取引モニタリング(特に出金/高額)。★**ウォレットレス(m0r)匿名モデルと衝突**→出金/閾値超で本人確認ゲートを挟み両立(製品思想とのトレードオフ)。
- **ジオブロック**: IP＋KYCベースで禁止法域(JP/US等・ライセンス準拠)を締め出す。
- **責任あるゲーミング**: 年齢確認・自己排除・上限設定・依存対策導線。
- **provably-fair**: D-4コミットリビール監査ログがライセンスの公正性要件を兼ねる。
- **プレイヤー資金の分別管理・払出保証**。

#### この姿勢での循環（クリアできる循環の埋め方）
有償転売が許容されるため原設計(高再生券を買う)が生き、循環の穴が埋まる:
```
NODE販売＋機能burn(D-1) ┐
Mad Hatter有償転売 1% ──┼→ プール(リザーブ平滑化) → 無料券＋有償券で参加 → 当選MORM
任意の有償参加 ────────┘                                    │
   ↑──── 当選者が再消費/再購入 → burn＋転売volume → 翌週プール ←┘
```
→ D-1ブレンドに**Mad Hatter手数料が主要原資として加わりプールが厚くなる**。リザーブ機構は暴落吸収。
※健全性のため、プールを賭博ターンオーバー依存にしすぎず、実需(NODE/機能消費)を背骨に残す。
...
### Phase 5 — MORM Mad Hatter（チケット二次流通・有償／D-2オフショア準拠）
- 出品/購入: `bs_tickets.status`=listed、`price`（出品者の言い値）、購入で`current_owner_addr`更新（`issuing_*`は不変）。
- **運営手数料1%**（`BS_MADHATTER_FEE`→プール原資の主要成分）。
- **エスクロー/決済方式**: MORMでのオンL1エスクロー（買い手→エスクロー→売り手、1%控除）。★**暗号資産交換業回避のためノンカストディ/オンチェーン型を優先**（運営がユーザー資金を預からない設計）。custodyモデル・返金経路・出品取消しを定義。
- **D-2コンプラゲート（必須）**: 購入/出金前に **KYC＋ジオブロック（禁止法域締め出し）＋年齢確認** を通す。KYC未了は無料券の参加までに限定し、**有償購入・出金は本人確認後**（ウォレットレスm0rとの両立ゲート）。責任あるゲーミング（上限/自己排除）。
- 抽選確定(freeze)後は出品ロック（開催直前の投機防止）。
- 完了条件: A出品→B購入→Bの券で参加→当選でBに賞金・発行元Aに配当、が通しで動く（概念FIG.2の再現）。かつ禁止法域/未KYCが有償導線でブロックされる。
...
- **D-2ジオブロック**: 有償導線を含むページはIP＋KYCベースで禁止法域(JP/US等)を締め出す（禁止法域ユーザーを入れないコンプラ制御。回避手段は作らない）。
- 完了条件: 実UIで週次サイクルが人間の目で回せる。

### Phase 7 — 監査 & ローンチ（D-2オフショア準拠）
- セキュリティ並列レビュー（前回2波方式）: 二重払い・view farm→確率・抽選操作・エスクロー抜き取り・treasury drain上限。
- **法務/ライセンス（着手前提）**: iGaming/暗号資産ゲーミング専門弁護士のサイン＋ライセンス法域での**法人設立・ライセンス取得**、**運営支配の所在設計**（★JP在住運営者への刑法波及リスクの解消）。KYC/AML・責任あるゲーミング・provably-fair の適合。
- テストネットで数週dry-run（発行→freeze→draw→settle→resale）。
- ローンチ判定（**ライセンス/法務サイン無しにローンチしない**）。
...
- **法規制（D-2オフショア）**: ライセンス法人＋KYC/AML＋ジオブロック＋責任あるゲーミング＋provably-fair。★JP在住運営者への刑法波及・暗号資産交換業（ノンカストディで回避）・弁護士/ライセンスをローンチゲートに。
...
2. ~~**D-2 法務の建て付け**~~ → **【決定】オフショア・ジオブロック型**（有償チケット維持＋ライセンス法人＋KYC/AML＋ジオブロック＋責任あるゲーミング）。循環は最強。残: **専門弁護士＋ライセンス法域・法人ストラクチャリング（★JP在住運営者への刑法波及の解消）**＝ローンチ前必須ゲート。
