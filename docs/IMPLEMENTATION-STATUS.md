...
>
> Last sync: 2026-05-07 (Phase 1-27i + 25Va/Vb/Vc + 22-Video + 26c/e/f/q/r/s/y + 24b throughput+incremental + portrait pivot + swipe feed + camera upload + wallet policy + Solidity audit + i18n EN/JA + onboarding guide + **Phase 28a /swap UI (ETH↔MORM)** + **Phase 28b USDC tab (ERC-20↔USDC.morm)** + **BRIDGE-DESIGN.md** + **Phase 13b-PoC M-of-N quorum bridge** + **Phase 30a-e Docker installer + ghcr.io v0.1.0 release** + **Phase 30f tunnel sidecars (cloudflared / Tailscale Funnel as compose profiles)** + **Phase 30g /admin peer discovery + producer registration template** + **public repo at github.com/Kemonos0424/morm + security audit (LAN IP scrub + unowned-DNS defang + SECURITY.md)** 完了 — OS-independent multi-host federated network with public ingress, browser-discoverable peer state, and self-onboarding producer registration)
...
| **/admin Phase 30g federation panels** | §11 (運用 — operator self-onboarding) | Phase 30g | `morm-player/passkey_morm.py` に 4 read-only admin endpoint 追加: `/api/admin/info` (L1 /info proxy)、`/api/admin/bootstrap` (L1 /bootstrap proxy)、`/api/admin/peer-discovery` (`seed_loader.load_peer_urls()` を server-side run + 各 peer の `/info` を 2s timeout で probe → `{url, source, alive, head_height, state_root, ms}` 配列)、`/api/admin/registration-template` (`/data/l1/producer.{pub,address}` または L1 /info から producer pubkey 抽出 → REGISTER_PRODUCER tx の inner_payload + `morm submit register-producer ...` CLI command + `already_registered` 判定 + 4-step instructions)。`morm-player/static/admin.{html,js}` で 2 新 card 追加 (Peer Discovery: source pill (explicit/local/baked/discovery)・alive dot・head height・state root・RTT 表示の table、10s auto-refresh / Become a Producer: producer pubkey + address + registered status pill (`pending — needs treasury approval` / `✓ registered`) + 850-char copy-able JSON template + 📋 copy button、15s refresh)。Phase 19 既存の SSH-based "Invite a New Node" card と Phase 24-UI Swarm Map は維持。`MORM_RPC` hardcode を `GATEWAY_URL = window.location.origin` 経由に切替 (Docker compose の internal network 互換) | ✅ E2E 検証 — fresh L1 8900 + preview gateway 8801 で 4 endpoint 全 200、admin page で 5 card render、producer pubkey f31f904a... + addr m0rplgxi... + status="pending — needs treasury approval" + template length 850 chars 確認、console errors 0 件、`{"summary":{"alive":0,"total":0,"by_source":{}}}` (空 seed list で fresh start も正常動作) |
| **Public ingress tunnel sidecars** | §11, §15 (NAT越え, 法人なし運用) | Phase 30f | `docker/morm-node.docker-compose.yml` に 2 つの opt-in profile 追加。**`tunnel-cf` (Cloudflare Tunnel)**: `cloudflare/cloudflared:latest`、`TUNNEL_TOKEN=${CLOUDFLARED_TOKEN:-}` を env で注入、`tunnel --no-autoupdate --metrics 0.0.0.0:2000 run` で gateway:8801 を Cloudflare anycast edge に publish。**`tunnel-ts` (Tailscale Funnel)**: `tailscale/tailscale:latest`、`TS_AUTHKEY` + `TS_HOSTNAME` + `cap_add: net_admin/sys_module` + `/dev/net/tun` device + `./data/tunnel-ts:/var/lib/tailscale` 永続 + `./tunnel-ts/funnel.json:/config/funnel.json:ro` で gateway:8801 を `<host>.<tailnet>.ts.net:443` (TLS terminated by Tailscale edge) に expose。両方とも depends_on: gateway healthy で起動順保証、profile gating により vanilla `up -d` には影響なし、profile 起動時に env が空なら sidecar が runtime fail で明確に通知。**3原則 (法人なし) との整合**: tunnel provider は browser-served HTML を見るだけ、tx 署名は client-side passkey + 2-of-2 XOR (Phase 7/9) なので tunnel 経由でも seed の半分しか暴露されない。`docker/.env.example` に CLOUDFLARED_TOKEN / TS_AUTHKEY / TS_HOSTNAME の env block 追加、`docker/tunnel-ts/funnel.json` template 同梱 (operator が hostname を編集して使う)、`morm-l1/ops/PUBLIC-INGRESS.md` 新規 (~110 行) で setup手順 + trust model + 「L1/Edge は意図的に internal 維持」設計判断を明文化、README.md に section + リンク追加 | ✅ Compose syntax validated — `docker compose config --services` で no-profile=3 services (l1/gateway/edge)、`--profile tunnel-cf`=4 (+tunnel-cf)、`--profile tunnel-ts`=4 (+tunnel-ts) 確認 |
